...
export function getVerticalBandLayout(
  ...
  const tileW = Math.max(1, meta.tileWidth | 0);
  const tileH = Math.max(1, meta.tileHeight | 0);
...
export function buildRuntimeStructureBandPieces(
  ...
  const layout = getVerticalBandLayout(input.spriteId, input.spriteWidth, input.spriteHeight, bandPx);
