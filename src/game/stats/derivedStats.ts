...
export function recomputeDerivedStats(w: World) {
    ...
    const hasLifeLess = (relicMods.lessMaxLife ?? 0) > 0;
    if (!hasLifeLess || !Number.isFinite(wAny.__relicBasePlayerHpMax)) {
        wAny.__relicBasePlayerHpMax = Math.max(1, Number.isFinite(w.playerHpMax) ? w.playerHpMax : 100);
    }
