...
function isWalkableBossTile(w: World, gx: number, gy: number): boolean {
...
export function findNearestWalkableSpawnGrid(
  ...
  if (isWalkableBossTile(w, bx, by)) return { gx: bx, gy: by };
  ...
  for (let r = 1; r <= maxRadiusTiles; r++) {
    for (let dx = -r; dx <= r; dx++) {
      ...
      const yTop = by - r;
      const yBottom = by + r;
      if (isWalkableBossTile(w, x, yTop)) return { gx: x, gy: yTop };
      if (isWalkableBossTile(w, x, yBottom)) return { gx: x, gy: yBottom };
    }
    for (let dy = -r + 1; dy <= r - 1; dy++) {
      const y = by + dy;
      const xLeft = bx - r;
      const xRight = bx + r;
      if (isWalkableBossTile(w, xLeft, y)) return { gx: xLeft, gy: y };
      if (isWalkableBossTile(w, xRight, y)) return { gx: xRight, gy: y };
    }
  }
