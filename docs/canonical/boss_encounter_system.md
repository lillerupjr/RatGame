...
3. **Activation**: `bossEncounterSystem(world, dt)` runs after progression on-kill sidecars and before zones/DOT work. It updates `arenaTileEffects`, then active encounters. Dormant encounters activate when player tile distance is within `boss.engageDistanceTiles`; while dormant they do not move, collide by contact, or cast.
